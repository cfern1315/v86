                ...
                else
                {
                    v86util.load_file(part_filename, {
                        done: async function done(buffer)
                        {
                            ...
                            if(this.is_zstd)
                            {
                                const dst = blocks.subarray(i * this.fixed_chunk_size, (i + 1) * this.fixed_chunk_size);
                                await this.zstd_decompress(dst, block);
                                this.handle_read((start_index + i) * this.fixed_chunk_size, this.fixed_chunk_size|0, dst);
                            }
                            else
                            {
                                blocks.set(block, i * this.fixed_chunk_size);
